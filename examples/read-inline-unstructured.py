...
with segyio.open(os.path.join(base_path, '0.sgy'), strict=False) as segyfile:
    ...
    slice_segy = np.zeros((max(xl_ids) - min(xl_ids) + 1, len(segyfile.samples)))
    for i, trace_id in enumerate(trace_ids):
        slice_segy[xl_ids[i] - min(xl_ids), :] = segyfile.trace[trace_id]
    print("segyio took", time.time() - t0)

with SgzReader(os.path.join(base_path, '0.sgz')) as reader:
    t0 = time.time()
    slice_sgz = reader.read_inline(LINE_NO-reader.ilines[0])
    print("SgzReader took", time.time() - t0)
    slice_sgz = slice_sgz[min(xl_ids) - reader.xlines[0]: max(xl_ids) - reader.xlines[0] + 1]
